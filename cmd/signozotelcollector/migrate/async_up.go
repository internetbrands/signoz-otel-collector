...
func registerAsyncUp(parentCmd *cobra.Command, logger *zap.Logger) {
	syncUpCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			up, err := newAsyncUp(config.Clickhouse.DSN, config.Clickhouse.Cluster, config.Clickhouse.Replication, config.MigrateSyncUp.Timeout, dbNames, logger)
			if err != nil {
				return err
			}
...
func newAsyncUp(dsn string, cluster string, replication bool, timeout time.Duration, dbNames schemamigrator.DatabaseNames, logger *zap.Logger) (*asyncUp, error) {
